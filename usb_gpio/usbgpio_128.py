    ...
    try:
        with serial.Serial(port_name, baud_rate, timeout=timeout) as ser_port:
            try:
                ...
                if isinstance(adc_channel, int) and 0 <= adc_channel <= 31:
                    ...
                    print(f"Error: adc_channel must be one of the digits between 0 and 31.")
            ...
            try:
                ...
                if isinstance(gpio_number, int) and 0 <= gpio_number <= 127:
                    if (int(gpio_number) < 100):
                    ...
                    print(f"Error: GPIO number must be one of the digits between 0 and 127.")
            except Exception as e:
                print(f"Error: GPIO number must be one of the digits between 0 and 127.") 
