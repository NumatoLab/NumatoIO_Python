    ...
    try:
        with serial.Serial(port_name, baud_rate, timeout=timeout) as ser_port:
            ...
            if isinstance(adc_channel, int) and 0 <= adc_channel <= 4:
                ...
                # Example 1: Read from ADC channel 1
            ...
            if isinstance(gpio_number, int) and 0 <= gpio_number <= 9:
                ...
                print("Error: GPIO number must be one of the digits between 0 and 9.")

            # Relay number
            relay_number = 14
            
            if isinstance(relay_number, int) and 0 <= relay_number <= 15:
            
                if (int(relay_number) < 10):
                    relay_number = str(relay_number)
                else:
                    relay_number = chr(55 + int(relay_number))

                # Example 6: ON Relay 14
                ...
                print("Error: relay_number must be one of the digits between 0 and 15.")
