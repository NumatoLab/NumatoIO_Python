...
def main():
    ...
    try:
        with serial.Serial(port_name, baud_rate, timeout=timeout) as ser_port:
            ...
            if isinstance(relay_number, int) and 0 <= relay_number <= 63:
            
                if (int(relay_number) < 10):
                    relay_number = str(relay_number).zfill(2)
            
                # Example 6: ON Relay 55
                ...
                print("Error: relay_number must be one of the digits between 0 and 63.")
